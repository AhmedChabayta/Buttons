...
import { PlayIcon as Play } from "@heroicons/react/outline";
import { motion, useCycle } from "framer-motion";
import Link from "next/link";

export default function Home() {
  const [colors, cycleColors] = useCycle(90, 180, 270);
  const [bright, cycleBright] = useCycle(100, 150, 200);
    ...
    <Layout>
      <div className="flex-col">
        <div className="flex w-full justify-center items-center space-x-10">
          <button className="flex flex-col" onClick={() => cycleColors()}>
            <p>HueRotation -{colors}- </p>
          </button>
          <button className="flex flex-col" onClick={() => cycleBright()}>
            <p>Brightness -{bright}- </p>
          </button>
        </div>
        <div
          className={`space-y-5 text-2xl w-screen h-screen flex flex-col hue-rotate-${colors} brightness-${bright} justify-center items-center pt-10 pl-10`}
        >
          <div className="flex space-x-10 items-center">
            ...
            <div className="3 rainbow group">
              <button
                className={`border-2 rounded-2xl ${
                  bright === 200 ? "border-black" : "border-gradient-to-br"
                } border-from-green-400 border-via-sky-500 border-to-purple-500 flex items-center before:h-full relative before:w-0 before:duration-[5s] after:duration-[5s] marker:duration-[5s]
              before:left-0 before:top-0 overflow-hidden hover:before:w-full before:transition-all before:ease-in-out
              before:absolute before:block before:content-[''] before:bg-gradient-to-br from-green-400 via-fuchsia-500 to-purple-500 bg-gradient-to-br before:blur
            text-black px-3 py-2 transition-all group-hover:hue-rotate-90 group-active:hue-rotate-180 before:brightness-150 group-hover:before:brightness-75`}
              >
                <p
                  className={`z-50 relative font-bold ${
                    bright === 200 ? "text-black" : "text-gray-50"
                  } transition-all`}
                >
                  SignIn
                </p>
            ...
            <div className="5 group">
              ...
              before:w-full before:h-[100%] before:rounded-[50px] before:bg-white px-5 py-2 bg-black group-hover:bg-gradient-to-br from-fuchsia-700 to-purple-500 hover:text-purple-500
              transition-all ease-linear group-hover:rounded-2xl rounded-[10px] text-black active:scale-[0.9]"
              >
                <p className="z-50 relative group-hover:animate-pulse">
                  SignIn
                </p>
              </button>
            </div>
          ...
          <div className="flex">
            <div className="flex group space-x-1">
              <button
                className="rainbow_top flex overflow-clip bg-blue-900 transition-all ease-linear duration-50 relative px-3 py-1 before:top-0
             before:absolute before:left-0 before:w-full before:h-1 before:transition-all before:ease-linear hover:before:h-full active:scale-[0.9]
             before:bg-gradient-to-r before:from-orange-500 before:via-red-500 to-orange-400"
              >
                <p className="z-50 text-white">SignIn</p>
              </button>
              <button
                className="flex before:hue-rotate-180 overflow-clip bg-blue-900 transition-all ease-linear duration-50 relative px-3 py-1 before:top-0
             before:absolute before:left-0 before:w-full before:h-1 before:transition-all before:ease-linear hover:before:h-full active:scale-[0.9]
             before:bg-gradient-to-r before:from-orange-500 before:via-red-500 to-orange-400"
              >
                <p className="z-50 text-white">Home</p>
              </button>
              <button
                className="skew flex before:hue-rotate-[145deg] overflow-clip bg-blue-900 transition-all ease-linear duration-50 relative px-3 py-1 before:top-0
             before:absolute before:left-0 before:w-full before:h-1 before:transition-all before:ease-linear hover:before:h-full active:scale-[0.9]
             before:bg-gradient-to-r before:from-orange-500 before:via-red-500 to-orange-400
             origin-top-left-right active:skew-z-12
             "
              >
                <p className="z-50 text-white">About</p>
              </button>
            </div>
          </div>
